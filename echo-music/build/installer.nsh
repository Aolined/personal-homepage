...
!define MINERADIO_INSTALL_MARKER ".echo-music-install-root"
...
!macro customInstall
  ...
  ${IfNot} ${Errors}
    FileWrite $0 "Echo Music install root$\r$\n"
    FileWrite $0 "appId=com.echomusic.desktop$\r$\n"
    FileClose $0
  ${EndIf}
!macroend
...
Function MineradioUseFirstAvailableInstallDir
  ...
  StrCpy $INSTDIR "C:\EchoMusic"
  Return

  driveD:
    StrCpy $INSTDIR "D:\EchoMusic"
    Return
  driveE:
    StrCpy $INSTDIR "E:\EchoMusic"
    Return
  driveF:
    StrCpy $INSTDIR "F:\EchoMusic"
    Return
  driveG:
    StrCpy $INSTDIR "G:\EchoMusic"
    Return
  driveH:
    StrCpy $INSTDIR "H:\EchoMusic"
    Return
  driveI:
    StrCpy $INSTDIR "I:\EchoMusic"
    Return
  driveJ:
    StrCpy $INSTDIR "J:\EchoMusic"
    Return
  driveK:
    StrCpy $INSTDIR "K:\EchoMusic"
    Return
  driveL:
    StrCpy $INSTDIR "L:\EchoMusic"
    Return
  driveM:
    StrCpy $INSTDIR "M:\EchoMusic"
    Return
  driveN:
    StrCpy $INSTDIR "N:\EchoMusic"
    Return
  driveO:
    StrCpy $INSTDIR "O:\EchoMusic"
    Return
  driveP:
    StrCpy $INSTDIR "P:\EchoMusic"
    Return
  driveQ:
    StrCpy $INSTDIR "Q:\EchoMusic"
    Return
  driveR:
    StrCpy $INSTDIR "R:\EchoMusic"
    Return
  driveS:
    StrCpy $INSTDIR "S:\EchoMusic"
    Return
  driveT:
    StrCpy $INSTDIR "T:\EchoMusic"
    Return
  driveU:
    StrCpy $INSTDIR "U:\EchoMusic"
    Return
  driveV:
    StrCpy $INSTDIR "V:\EchoMusic"
    Return
  driveW:
    StrCpy $INSTDIR "W:\EchoMusic"
    Return
  driveX:
    StrCpy $INSTDIR "X:\EchoMusic"
    Return
  driveY:
    StrCpy $INSTDIR "Y:\EchoMusic"
    Return
  driveZ:
    StrCpy $INSTDIR "Z:\EchoMusic"
    Return
FunctionEnd
...
Function MineradioValidateInstallDir
  ...
  ${If} $1 == ":"
    ...
    ${OrIf} $0 == "c"
      ...
      ${AndIf} $3 != "1"
      ${AndIf} $4 != "1"
        MessageBox MB_ICONSTOP|MB_OK "检测到这台电脑还有 D-Z 盘，Echo Music 不安装到 C 盘。请改选 D 盘或其它非 C 盘的 EchoMusic 文件夹。$\r$\n$\r$\n如果电脑只有 C 盘，安装器会自动放行 C:\EchoMusic。"
        Abort
      ${EndIf}
    ${EndIf}
  ${EndIf}
  ...
  ${OrIf} $1 != "\EchoMusic"
  ${AndIf} $1 != "\echomusic"
    MessageBox MB_ICONSTOP|MB_OK "安装目录必须是独立的 EchoMusic 文件夹。请选择一个上级目录，安装器会自动创建 EchoMusic 子文件夹。"
    Abort
  ${EndIf}
  ...
  MessageBox MB_ICONSTOP|MB_OK "为避免卸载时误删其它文件，Echo Music 不能安装到已有文件的非专属目录。请新建或选择一个空的 EchoMusic 文件夹。$\r$\n$\r$\n当前路径：$INSTDIR"
  Abort
...
Function MineradioWelcomeShow
  ...
  ${NSD_CreateLabel} 22u 20u 82u 10u "ECHO MUSIC"
  Pop $0
  SendMessage $0 ${WM_SETFONT} $MineradioSmallFont 1
  SetCtlColors $0 "3257F7" "FFFFFF"

  ${NSD_CreateLabel} 22u 42u 226u 30u "Echo Music 安装"
  Pop $0
  ...
  ${NSD_CreateLabel} 22u 96u 238u 24u "为这台电脑安装 Echo Music。默认安装到 D:\EchoMusic，下一步可以自由选择其它位置。"
  Pop $0
...
Function MineradioDirectoryBrowse
  nsDialogs::SelectFolderDialog "选择 Echo Music 安装文件夹" "$INSTDIR"
  Pop $0
  ${If} $0 != error
  ${AndIf} $0 != ""
    Push "$0"
...
Function MineradioDirectoryShow
  ...
  ${NSD_CreateLabel} 22u 122u 238u 12u "默认推荐：D:\EchoMusic；选盘符会自动建文件夹。"
  Pop $0
  SendMessage $0 ${WM_SETFONT} $MineradioSmallFont 1
...
Function un.MineradioValidateUninstallDir
  ...
  Pop $1
  ${If} $0 != $1
    MessageBox MB_OK|MB_ICONSTOP "当前卸载路径不是 Echo Music 专属目录，已阻止卸载以避免误删其它文件。$\r$\n$\r$\n当前路径：$INSTDIR$\r$\n安全路径应为：$0"
    SetErrorLevel 2
    Quit
  ${EndIf}
  StrCpy $INSTDIR "$0"
  ...
  Pop $0
  ${If} $0 != "1"
    MessageBox MB_OK|MB_ICONSTOP "无法确认当前目录属于 Echo Music，已阻止卸载以避免误删其它文件。$\r$\n$\r$\n当前路径：$INSTDIR"
    SetErrorLevel 2
    Quit
  ${EndIf}
FunctionEnd
