...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Echo Music icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'EchoMusic';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Echo Music executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Echo Music icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Echo Music resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Echo Music',
    '--set-version-string', 'ProductName', 'Echo Music',
    '--set-version-string', 'CompanyName', 'Echo Music',
