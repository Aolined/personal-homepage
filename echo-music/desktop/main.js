...
const APP_NAME = 'Echo Music';
const APP_USER_MODEL_ID = 'com.echomusic.desktop';
const APP_ICON_ICO = path.join(__dirname, '..', 'build', 'icon.ico');
const NETEASE_LOGIN_PARTITION = 'persist:mineradio-netease-login';
const NETEASE_LOGIN_URL = 'https://music.163.com/#/login';
const QQ_LOGIN_PARTITION = 'persist:mineradio-qqmusic-login';
const QQ_LOGIN_URL = 'https://y.qq.com/n/ryqq/profile';
const QISHUI_LOGIN_PARTITION = 'persist:echo-qishui-login';
const QISHUI_LOGIN_URL = 'https://www.douyin.com/user/self?from_tab_name=main';
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'Echo Music immersive desktop player',
...
async function getQishuiMusicLoginStatus() {
...
async function openQishuiMusicLoginWindow(owner) {
  const cookieSession = session.fromPartition(QISHUI_LOGIN_PARTITION);
  const initialStatus = await getQishuiMusicLoginStatus();
  if (initialStatus.loggedIn) return { ...initialStatus, reused: true };
  ...
  return new Promise((resolve) => {
    ...
    const checkCookies = async () => {
      try {
        const cookie = await readQishuiLoginCookieHeader(cookieSession);
        if (qishuiCookieHasLogin(cookie)) finish({ ok: true, provider: 'qishui', loggedIn: true, persisted: true });
      } catch (error) {
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      try {
        ...
        if (target.protocol === 'https:' && isQishuiCookieDomain(target.hostname)) {
          loginWindow.loadURL(url).catch((error) => console.warn('Qishui login popup navigation failed:', error.message));
        } else if (/^https?:\/\//i.test(url)) {
          shell.openExternal(url).catch(() => {});
        }
      } catch (_) {}
    ...
    loginWindow.webContents.on('did-finish-load', () => {
      ...
      loginWindow.webContents.executeJavaScript(`
        setTimeout(() => {
          ...
          const loginNode = nodes.find((node) => {
            const text = (node.textContent || '').trim();
            if (!/登录|扫码登录/.test(text)) return false;
            const rect = node.getBoundingClientRect();
            return rect.width > 0 && rect.height > 0;
          });
          if (loginNode) loginNode.click();
    ...
    loginWindow.on('closed', async () => {
      ...
      if (pollTimer) clearInterval(pollTimer);
      try {
        const status = await getQishuiMusicLoginStatus();
        resolve(status.loggedIn ? status : { ok: false, provider: 'qishui', loggedIn: false, cancelled: true, message: '汽水登录窗口已关闭' });
      } catch (error) {
        resolve({ ok: false, provider: 'qishui', loggedIn: false, error: error.message || '汽水登录窗口已关闭' });
      }
    });

    pollTimer = setInterval(checkCookies, 1200);
    loginWindow.loadURL(QISHUI_LOGIN_URL).catch((error) => finish({ ok: false, provider: 'qishui', loggedIn: false, error: error.message }));
  });
}
...
async function clearQishuiMusicLoginSession() {
  ...
  return { ok: true, provider: 'qishui', loggedIn: false };
}
...
function toggleMaximizedWindow(win) {
  if (!win || win.isDestroyed()) return;
  if (win.isMaximized()) win.unmaximize();
  else win.maximize();
  sendWindowState(win);
...
function createDesktopLyricsWindow(payload = {}) {
  ...
  desktopLyricsWindow = new BrowserWindow({
    ...
    skipTaskbar: true,
    show: false,
    title: 'Echo Music Desktop Lyrics',
    webPreferences: {
...
ipcMain.handle('desktop-window-toggle-maximize', (event) => {
  toggleMaximizedWindow(getSenderWindow(event));
});
...
ipcMain.handle('mineradio-export-json-file', async (event, payload = {}) => {
  try {
    ...
    const defaultName = String(payload.defaultName || 'echo-music-export.json').replace(/[\\/:*?"<>|]+/g, '-');
    const result = await dialog.showSaveDialog(owner, {
      title: '导出 Echo Music 存档',
...
ipcMain.handle('qishui-music-login-status', async () => {
  return getQishuiMusicLoginStatus();
});
