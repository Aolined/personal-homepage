...
## Echo Music - Unreleased

- 恢复独立的 Echo Music Web 官网入口：`/` 展示品牌首页，`/app` 运行完整播放器，避免官网与播放器首页互相覆盖。
- 修复网页版登录二维码上游失败时出现白色破图，以及天气不可用时错误显示 `0°/0%` 的问题。
- 将桌面产品名称、App ID、快捷方式、EXE 和安装包命名切换为 Echo Music。
- 新增 Echo Signal Stage 主题、回声环品牌标志和可用首页，保留原播放器与 Three.js 视觉逻辑。
- 新增多尺寸 Windows 图标和 Echo Music 安装器资源生成脚本。
- 关闭上游 Mineradio 自动更新配置，避免二改版本被上游 Release 覆盖。
- 新增品牌回归测试和仅监听本机的 Web 预览入口。
- 新增正式 Echo Music Web 运行模式，浏览器端自动隐藏桌面歌词、壁纸和全局热键等桌面专属控制。
- Web 模式默认仅监听本机，增加安全响应头、同源限制、静态路径保护和音乐媒体代理域名限制。
- 新增汽水音乐分享导入：解析官方短链或分享页中的单曲/歌单信息，并匹配网易云音乐或 QQ 音乐可播放版本。
- 汽水导入仅允许官方分享域名，限制最多 24 首、2 路并发匹配，并排除 UGC 视频分享和低可信度同名结果。
