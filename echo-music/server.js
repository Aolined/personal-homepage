...
const WEB_RUNTIME = process.env.ECHO_RUNTIME === 'web';
const PUBLIC_GUEST_MODE = isPublicGuestMode(WEB_RUNTIME);
const TRUST_PROXY = /^(1|true)$/i.test(String(process.env.TRUST_PROXY || ''));
const RESPONSE_SECURITY_HEADERS = responseSecurityHeaders(WEB_RUNTIME);
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const MAX_COVER_PROXY_BYTES = 3 * 1024 * 1024;
const IMAGE_CONTENT_TYPE_RE = /^image\/(?:jpeg|jpg|png|webp|gif|avif|bmp)(?:;|$)/i;
const PUBLIC_MAX_AUDIO_STREAMS = 12;
const publicGuestRateLimiter = createFixedWindowRateLimiter();
let publicAudioStreams = 0;
...
const WINDOWS_INSTALLER_VERSION = String(APP_VERSION).replace(/[^0-9A-Za-z._-]/g, '-');
const WINDOWS_INSTALLER_NAME = `Echo-Music-${WINDOWS_INSTALLER_VERSION}-Setup.exe`;
const WINDOWS_INSTALLER_FILE = path.join(__dirname, 'dist', WINDOWS_INSTALLER_NAME);
...
const CHINA_WEATHER_SEARCH_URL = 'http://toy1.weather.com.cn/search';
const CHINA_WEATHER_OBSERVATION_URL = 'http://d1.weather.com.cn/sk_2d/';
const WEATHER_REVERSE_CACHE_TTL_MS = 60 * 60 * 1000;
const CHINA_WEATHER_CACHE_TTL_MS = 5 * 60 * 1000;
...
const weatherReverseLocationCache = new Map();
const chinaWeatherStationCache = new Map();
const chinaWeatherObservationCache = new Map();
...
let userCookie = '';
try { if (!PUBLIC_GUEST_MODE && fs.existsSync(COOKIE_FILE)) userCookie = fs.readFileSync(COOKIE_FILE, 'utf8').trim(); }
catch (e) { userCookie = ''; }
function saveCookie(c) {
  if (PUBLIC_GUEST_MODE) { userCookie = ''; return; }
  userCookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  try { fs.writeFileSync(COOKIE_FILE, userCookie); } catch (e) {}
}

let qqCookie = '';
try { if (!PUBLIC_GUEST_MODE && fs.existsSync(QQ_COOKIE_FILE)) qqCookie = fs.readFileSync(QQ_COOKIE_FILE, 'utf8').trim(); }
catch (e) { qqCookie = ''; }
function saveQQCookie(c) {
  if (PUBLIC_GUEST_MODE) { qqCookie = ''; return; }
  qqCookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  try { fs.writeFileSync(QQ_COOKIE_FILE, qqCookie); } catch (e) {}
}

// ---------- 工具 ----------
const COMPRESSIBLE_STATIC_EXTENSIONS = new Set(['.html', '.js', '.css', '.json', '.svg']);
function staticCacheControl(filePath, ext) {
  if (ext === '.html') return 'no-cache, must-revalidate';
  if (filePath.includes(path.sep + 'vendor' + path.sep)
    || filePath.includes(path.sep + 'assets' + path.sep)
    || ['.png', '.jpg', '.jpeg', '.webp', '.svg', '.ico', '.bin'].includes(ext)) {
    return 'public, max-age=604800';
  }
  return 'no-cache, must-revalidate';
}
function staticEncoding(req, ext, size) {
  if (!COMPRESSIBLE_STATIC_EXTENSIONS.has(ext) || size < 1024) return '';
...
function serveStatic(req, res, filePath) {
  ...
  fs.readFile(filePath, (err, data) => {
    if (err) { res.writeHead(404, RESPONSE_SECURITY_HEADERS); res.end('Not Found'); return; }
    const encoding = staticEncoding(req, ext, data.length);
    const headers = {
      ...RESPONSE_SECURITY_HEADERS,
      'Content-Type': MIME[ext] || 'text/plain',
      'Cache-Control': staticCacheControl(filePath, ext),
      'Vary': 'Accept-Encoding',
    };
    const send = (body, appliedEncoding) => {
    ...
    if (encoding === 'br') {
      ...
      }, (compressErr, body) => send(compressErr ? data : body, compressErr ? '' : 'br'));
      return;
    }
    if (encoding === 'gzip') {
      zlib.gzip(data, { level: 6 }, (compressErr, body) => send(compressErr ? data : body, compressErr ? '' : 'gzip'));
      return;
    }
    send(data);
  });
}
function serveDownload(req, res, filePath, downloadName) {
  fs.stat(filePath, (err, stat) => {
    if (err || !stat.isFile()) {
      res.writeHead(404, { ...RESPONSE_SECURITY_HEADERS, 'Content-Type': 'text/plain; charset=utf-8' });
    ...
    if (range) {
      const requestedStart = range[1] === '' ? NaN : Number(range[1]);
      const requestedEnd = range[2] === '' ? NaN : Number(range[2]);
      if (Number.isFinite(requestedStart)) start = requestedStart;
      if (Number.isFinite(requestedEnd)) end = requestedEnd;
      ...
      if (start < 0 || end < start || start >= stat.size) {
        res.writeHead(416, { ...RESPONSE_SECURITY_HEADERS, 'Content-Range': `bytes */${stat.size}` });
        res.end();
        return;
      }
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.echo && pkg.echo.update)
...
async function resolveChinaWeatherStation(city) {
  const query = normalizeChineseCityName(city);
  if (!query || !/[\u3400-\u9fff]/u.test(query)) {
    const error = new Error('CHINA_WEATHER_CITY_NOT_FOUND');
    error.code = 'CHINA_WEATHER_CITY_NOT_FOUND';
    throw error;
  }
  if (chinaWeatherStationCache.has(query)) return chinaWeatherStationCache.get(query);
  const u = new URL(CHINA_WEATHER_SEARCH_URL);
  u.searchParams.set('cityname', query);
  const payload = await requestText(u.toString(), {
    family: 4,
    timeoutMs: 4000,
    headers: { Referer: 'http://www.weather.com.cn/', 'User-Agent': UA },
  });
  const station = parseWeatherComSearch(payload, query);
  chinaWeatherStationCache.set(query, station);
  return station;
}

async function fetchChinaWeatherObservation(city) {
  const station = await resolveChinaWeatherStation(city);
  const cached = chinaWeatherObservationCache.get(station.code);
  if (cached && Date.now() - cached.cachedAt < CHINA_WEATHER_CACHE_TTL_MS) return cached.observation;
  const payload = await requestText(CHINA_WEATHER_OBSERVATION_URL + station.code + '.html', {
    family: 4,
    timeoutMs: 4000,
    headers: { Referer: 'http://www.weather.com.cn/', 'User-Agent': UA },
  });
  const observation = parseWeatherComObservation(payload);
  chinaWeatherObservationCache.set(station.code, { cachedAt: Date.now(), observation });
  return observation;
}

function applyChinaWeatherObservation(weather, observation) {
  if (!weather || !observation) return weather;
  weather.provider = 'weather.com.cn+open-meteo';
  weather.observationProvider = observation.provider;
  weather.label = observation.label;
  weather.weatherCode = observation.weatherCode;
  weather.temperature = observation.temperature;
  weather.apparentTemperature = observation.apparentTemperature;
  weather.humidity = observation.humidity;
  weather.precipitation = observation.precipitation;
  if (Number.isFinite(observation.windSpeed)) weather.windSpeed = observation.windSpeed;
  weather.observationTime = observation.observationTime;
  return weather;
}
...
async function fetchOpenMeteoWeather(params) {
  ...
  const chinaObservationPromise = fetchChinaWeatherObservation(location.name).catch(error => {
    console.warn('[ChinaWeather]', error.code || error.message);
    return null;
  });
  ...
  applyChinaWeatherObservation(weather, await chinaObservationPromise);
...
async function fetchReverseWeatherLocation(lat, lon) {
  const coordinates = parseCoordinates(lat, lon);
  const cacheKey = reverseLocationCacheKey(coordinates.latitude, coordinates.longitude);
  const cached = weatherReverseLocationCache.get(cacheKey);
  if (cached && Date.now() - cached.cachedAt < WEATHER_REVERSE_CACHE_TTL_MS) return cached.location;

  const u = new URL(WEATHER_REVERSE_LOCATION_URL);
  u.searchParams.set('latitude', String(coordinates.latitude));
  u.searchParams.set('longitude', String(coordinates.longitude));
  ...
  const body = await requestJson(u.toString(), {
    ...
    headers: {
      ...
      'User-Agent': 'EchoMusic/' + APP_VERSION + ' (local music player)',
    },
  });
  const location = normalizeReverseLocation(body, coordinates.latitude, coordinates.longitude);
  if (weatherReverseLocationCache.size >= 200) {
    weatherReverseLocationCache.delete(weatherReverseLocationCache.keys().next().value);
  }
  weatherReverseLocationCache.set(cacheKey, { cachedAt: Date.now(), location });
  return location;
}
...
async function matchQishuiTrack(track) {
  const query = [track && track.name, track && track.artist].filter(Boolean).join(' ').trim();
  if (!query) return { source: track, song: null };
  const result = await Promise.allSettled([
    handleSearch(query, 8),
    handleQQSearch(query, 8),
  ]);
  const netease = result[0].status === 'fulfilled' ? result[0].value : [];
  const qq = result[1].status === 'fulfilled' ? result[1].value : [];
  const song = pickBestImportedSong(track, netease.concat(qq));
  return {
    source: track,
    song: song ? {
      ...song,
      importedFrom: 'qishui',
      importedTrack: { id: track.id || '', name: track.name || '', artist: track.artist || '' },
    } : null,
  };
}
...
async function handleQishuiImport(text) {
  const share = await fetchQishuiShare(text, { timeoutMs: 10000 });
  if (!share.tracks.length || share.kind === 'unsupported') {
    const error = new Error('QISHUI_SHARE_UNSUPPORTED');
    error.code = 'QISHUI_SHARE_UNSUPPORTED';
    throw error;
  }
  const sourceTracks = share.tracks.slice(0, 24);
  const matches = await mapWithConcurrency(sourceTracks, 2, matchQishuiTrack);
  return {
    provider: 'qishui',
    kind: share.kind,
    title: share.title || (share.kind === 'playlist' ? '汽水歌单' : sourceTracks[0].name),
    sourceUrl: share.sourceUrl,
    total: sourceTracks.length,
    matched: matches.filter(item => item.song).length,
    songs: matches.filter(item => item.song).map(item => item.song),
    items: matches,
    truncated: share.tracks.length > sourceTracks.length,
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/healthz') {
    sendJSON(res, { status: 'ok', service: 'echo-music', publicGuest: PUBLIC_GUEST_MODE });
    return;
  }

  if (pn === '/runtime-config.js') {
    const body = `window.ECHO_RUNTIME_CONFIG=${JSON.stringify({ mode: WEB_RUNTIME ? 'web' : 'desktop', publicGuest: PUBLIC_GUEST_MODE })};`;
    res.writeHead(200, {
      ...RESPONSE_SECURITY_HEADERS,
  ...
  if (PUBLIC_GUEST_MODE) {
    const routeResponse = publicGuestRouteResponse(pn);
    if (routeResponse) {
      sendJSON(res, routeResponse.body, routeResponse.status);
      return;
    }
    if (!['GET', 'HEAD'].includes(req.method)) {
      sendJSON(res, { error: 'METHOD_NOT_ALLOWED', message: '公开访客模式仅接受只读请求' }, 405, { Allow: 'GET, HEAD' });
      return;
    }
    if (pn.startsWith('/api/')) {
      const rate = publicGuestRateLimiter.consume(getClientAddress(req, TRUST_PROXY));
      const rateHeaders = {
      ...
      if (!rate.allowed) {
        sendJSON(res, { error: 'RATE_LIMITED', message: '请求过于频繁，请稍后再试' }, 429, {
          ...rateHeaders,
          'Retry-After': String(rate.retryAfterSeconds),
        });
        return;
      }
      Object.entries(rateHeaders).forEach(([name, value]) => res.setHeader(name, value));
    }
  }

  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'echo-music',
      productName: APP_PACKAGE.productName || 'Echo Music',
      version: APP_VERSION,
      runtime: runtimeMetadata(WEB_RUNTIME, PUBLIC_GUEST_MODE),
      update: {
  ...
  if (pn === '/api/weather/reverse-location') {
    if (req.method !== 'GET') {
      sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED', location: null }, 405);
      return;
    }
    try {
      const location = await fetchReverseWeatherLocation(
        url.searchParams.get('lat'),
        url.searchParams.get('lon')
      );
      sendJSON(res, { ok: true, location });
    } catch (err) {
      const invalid = err && err.code === 'INVALID_COORDINATES';
      console.warn('[WeatherReverseLocation]', err.code || err.message);
      sendJSON(res, { ok: false, error: err.code || err.message, location: null }, invalid ? 400 : 502);
    }
    return;
  }

  // ---------- 搜索 ----------
  if (pn === '/api/import/qishui') {
    if (req.method !== 'POST') {
      sendJSON(res, { provider: 'qishui', error: 'METHOD_NOT_ALLOWED' }, 405);
      return;
    }
    try {
      const body = await readRequestBody(req);
      const text = String(body.text || body.share || '').trim();
      if (!text || text.length > 4096) {
        sendJSON(res, { provider: 'qishui', error: 'INVALID_QISHUI_SHARE_TEXT' }, 400);
        return;
      }
      sendJSON(res, await handleQishuiImport(text));
    } catch (err) {
      const badRequest = err.code === 'INVALID_QISHUI_SHARE_URL';
      const unsupported = err.code === 'QISHUI_SHARE_UNSUPPORTED';
      console.warn('[QishuiImport]', err.code || err.message);
      sendJSON(res, {
        provider: 'qishui',
        error: err.code || 'QISHUI_IMPORT_FAILED',
        message: unsupported ? '该汽水分享暂不是可导入的歌曲或歌单' : '汽水分享解析失败，请检查链接后重试',
        songs: [],
        items: [],
      }, badRequest ? 400 : (unsupported ? 422 : 502));
    }
    return;
  }
  ...
  if (pn === '/api/podcast/dj-beatmap') {
    try {
      ...
      if (!audioUrl || !isAllowedWebProxyUrl(audioUrl, 'audio')) {
  ...
  if (pn === '/api/login/qr/key') {
    try {
      ...
      if (!key) {
        console.warn('[LoginQrKey] upstream response did not contain a key');
        sendJSON(res, { error: 'NETEASE_QR_KEY_UNAVAILABLE' }, 502);
        return;
      }
      sendJSON(res, { key });
    } catch (err) {
      console.warn('[LoginQrKey]', err && (err.message || err.code) || err);
      sendJSON(res, { error: 'NETEASE_QR_KEY_UNAVAILABLE' }, 502);
    }
    return;
  }
  ...
  if (pn === '/api/login/qr/create') {
    try {
      ...
      if (!d || !d.qrimg) {
        sendJSON(res, { error: 'NETEASE_QR_CREATE_UNAVAILABLE' }, 502);
        return;
      }
      sendJSON(res, { img: d && d.qrimg, url: d && d.qrurl });
    } catch (err) {
      console.warn('[LoginQrCreate]', err && (err.message || err.code) || err);
      sendJSON(res, { error: 'NETEASE_QR_CREATE_UNAVAILABLE' }, 502);
    }
    return;
  }
  ...
  if (pn === '/api/cover') {
    try {
      ...
      if (!coverUrl || !isAllowedWebProxyUrl(coverUrl, 'cover')) {
        res.writeHead(400, RESPONSE_SECURITY_HEADERS);
        res.end('Invalid cover url');
        return;
      }
      const resp = await fetch(coverUrl, { headers: { 'User-Agent': UA, 'Referer': 'https://music.163.com/' } });
      const ct = resp.headers.get('content-type') || '';
      if (!resp.ok || !IMAGE_CONTENT_TYPE_RE.test(ct)) {
        res.writeHead(resp.ok ? 415 : resp.status, RESPONSE_SECURITY_HEADERS);
        res.end('Invalid cover response');
        return;
      }
      const declaredLength = Number(resp.headers.get('content-length') || 0);
      if (declaredLength > MAX_COVER_PROXY_BYTES) {
        res.writeHead(413, RESPONSE_SECURITY_HEADERS);
        res.end('Cover image too large');
        return;
      }
      const body = Buffer.from(await resp.arrayBuffer());
      if (body.length > MAX_COVER_PROXY_BYTES) {
        res.writeHead(413, RESPONSE_SECURITY_HEADERS);
        res.end('Cover image too large');
        return;
      }
      const hdr = {
        ...RESPONSE_SECURITY_HEADERS,
        'Content-Type': ct,
        'Cross-Origin-Resource-Policy': WEB_RUNTIME ? 'same-origin' : 'cross-origin',
  ...
  // ---------- 音频代理 (支持 Range) ----------
  if (pn === '/api/audio') {
    if (PUBLIC_GUEST_MODE && publicAudioStreams >= PUBLIC_MAX_AUDIO_STREAMS) {
      sendJSON(res, { error: 'AUDIO_CAPACITY', message: '当前播放人数较多，请稍后重试' }, 503, { 'Retry-After': '5' });
      return;
    }
    if (PUBLIC_GUEST_MODE) publicAudioStreams += 1;
    try {
      const audioUrl = url.searchParams.get('url');
      if (!audioUrl || !isAllowedWebProxyUrl(audioUrl, 'audio')) {
        res.writeHead(400, RESPONSE_SECURITY_HEADERS);
    ...
    finally { if (PUBLIC_GUEST_MODE) publicAudioStreams = Math.max(0, publicAudioStreams - 1); }
    return;
  }

  // ---------- 静态资源 ----------
  if (pn === '/download/windows') {
    serveDownload(req, res, WINDOWS_INSTALLER_FILE, WINDOWS_INSTALLER_NAME);
    return;
  }

  if (pn === '/favicon.ico') {
    serveStatic(req, res, path.join(__dirname, 'build', 'icon.ico'));
    return;
  }

  let filePath = pn;
  if (pn === '/') filePath = WEB_RUNTIME ? '/landing.html' : '/index.html';
  if (WEB_RUNTIME && (pn === '/app' || pn === '/app/')) {
    if (pn === '/app/') {
      res.writeHead(308, { ...RESPONSE_SECURITY_HEADERS, Location: '/app' });
      res.end();
      return;
    }
    filePath = '/index.html';
  ...
  if (filePath !== publicRoot && !filePath.startsWith(publicRoot + path.sep)) {
    res.writeHead(404, RESPONSE_SECURITY_HEADERS);
    res.end('Not Found');
    return;
  }
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log(' Echo Music ' + (WEB_RUNTIME ? 'Web' : 'Desktop') + '  →  http://' + HOST + ':' + PORT);
  console.log(' 登录态: ' + (PUBLIC_GUEST_MODE ? '公开访客模式' : (userCookie ? '已登录(cookie已加载)' : '未登录')));
  console.log('======================================================');
});

server.requestTimeout = PUBLIC_GUEST_MODE ? 30_000 : server.requestTimeout;
server.headersTimeout = PUBLIC_GUEST_MODE ? 10_000 : server.headersTimeout;
